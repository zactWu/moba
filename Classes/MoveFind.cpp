...
//	log("use mapint");
	...
	for (int i = 0; i < movemarknums; i++) {
		...
		movemarks.push_back(a);
	}
	
	for (int i = 0; i<movemarks.size(); i++) {
		for (int j = i + 1; j < movemarks.size(); j++) {
			if (canto(movemarks[i].position, movemarks[j].position,map)) {
				movemarks[i].can_be.push_back(j);
				movemarks[j].can_be.push_back(i);
